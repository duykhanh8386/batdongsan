...
public class HomeController {
    ...
    public ModelAndView getHomePage(@RequestParam(value = "salesFilter", defaultValue = "today") String salesFilter,
                                    @RequestParam(value = "statusFilter", defaultValue = "thisMonth") String statusFilter) {
        ...
        //
        modelAndView.addObject("indicators", transactionService.getRadarIndicators());
        modelAndView.addObject("series", transactionService.getRadarSeries(statusFilter));
        modelAndView.addObject("statusFilter", statusFilter); // Để hiển thị bộ lọc hiện tại
    ...
    @ResponseBody
    public Map<String, Object> getReportData(@RequestParam String range) {
        return transactionService.getReportData(range);
    }
